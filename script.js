...
// ========== ВХОД И РЕГИСТРАЦИЯ (localStorage + Neon через API) ==========
...
async function apiRequest(method, body) {
    try {
        ...
        if (!res.ok) {
            ...
            return { ok: false, error: err.error || 'API error' };
        }
...
async function renderAdminUsersList() {
    ...
    localUsers.forEach((u) => {
        ...
        merged.push({ ...u, _source: 'local' });
    });

    syncedUsers.forEach((u) => {
        if (!allEmails.has(u.email)) {
            allEmails.add(u.email);
            merged.push({ ...u, _source: 'telegram' });
        }
    });
    ...
    html += merged.map((user) => {
        const canBan = user._source === 'local' || user._source === 'neon';
        const banned = user.blocked;
        const sourceLabel = user._source === 'neon' ? '<br><span style="color:#6366f1;font-size:11px;">☁ Neon (все устройства)</span>' :
                           user._source === 'telegram' ? '<br><span style="color:#0d9488;font-size:11px;">📱 Из Telegram</span>' : '';
        return `
